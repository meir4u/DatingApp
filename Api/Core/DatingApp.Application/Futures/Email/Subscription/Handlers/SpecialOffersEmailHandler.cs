...
namespace DatingApp.Application.Futures.Email.Subscription.Handlers
{
    ...
    /// </summary>
    public class SpecialOffersEmailHandler : BaseEmailHandler<SpecialOffersEmailRequest, SpecialOffersEmailResponse>
    {
        protected override string _templateName { get; set; } = "SpecialOffers";

        public SpecialOffersEmailHandler(IUnitOfWork unitOfWork, IEnhancedEmailService enhancedEmailService)
             : base(unitOfWork, enhancedEmailService)
        {
        }
        public override async Task<SpecialOffersEmailResponse> Handle(SpecialOffersEmailRequest request, CancellationToken cancellationToken)
        {
            var response = new SpecialOffersEmailResponse();
            try
            {
